...
export function usePageTheme(): PageThemeConfig {
  ...
  const themeKey = (() => {
    ...
    if (p.startsWith('/admin')) p = p.replace('/admin', '/dashboard');
    else if (p.startsWith('/officer')) p = p.replace('/officer', '/dashboard');
    else if (p.startsWith('/citizen')) p = p.replace('/citizen', '/dashboard');
    return p;
  })();
