...
import { homePathForRole } from '@shared/constants/userPortalPaths';
    ...
    if (!isLoading && user) {
      if (IS_ADMIN_SURFACE) {
        if (user.role === 'admin') {
          ...
          window.location.assign(getUserDevUrl(homePathForRole(user.role)));
        }
        return;
      }
    ...
    try {
      ...
      const go = () => {
        if (loggedIn.role === 'admin') {
          ...
          window.location.assign(getUserDevUrl(homePathForRole(loggedIn.role)));
        } else {
