...
function rewriteRemoteMediaUrl(url: string): string | null {
  try {
    ...
    // django-storages may omit the /media prefix depending on AWS_LOCATION.
    const locationMatch = path.match(/\/(?:media\/)?((?:ai|fines|violations|signs|cctv|users|vehicles|profiles)\/.+)$/i);
    if (locationMatch?.[1]) {
...
export function getProfileImageSrc(profileImage?: string | null): string | null {
  ...
  let token = profileImage;
  try {
    const cleaned = profileImage.split('?')[0];
    token = cleaned.includes('/') ? (cleaned.split('/').pop() || cleaned) : cleaned;
  } catch {
    /* keep */
  }
