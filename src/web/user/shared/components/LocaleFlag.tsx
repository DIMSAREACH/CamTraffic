import cambodiaFlagUrl from '@shared/assets/flags/cambodia-flag.svg?url';
...
type LocaleFlagProps = {
  locale: Locale;
  className?: string;
  /** inline = navbar trigger; fill = fills parent (dropdown slot) */
  size?: 'sm' | 'fill';
};

/** UK Union Jack (viewBox 55.2 × 38.4). */
function UkFlagSvg() {
  return (
    <svg
...
function CambodiaFlagImg({ className }: { className?: string }) {
  return (
    <img
      src={cambodiaFlagUrl}
      alt=""
      draggable={false}
      className={cn('locale-flag__img', className)}
    />
  );
}

/** Flag icon for English (UK) and Khmer (Cambodia). */
export function LocaleFlag({ locale, className, size = 'sm' }: LocaleFlagProps) {
  const isKh = locale === 'km';
  const sizeClass = size === 'fill' ? 'locale-flag--fill' : 'locale-flag--sm';
  const localeClass = isKh ? 'locale-flag--kh' : 'locale-flag--en';

  return (
    <span className={cn('locale-flag', sizeClass, localeClass, className)} aria-hidden>
      {isKh ? <CambodiaFlagImg /> : <UkFlagSvg />}
    </span>
  );
}
