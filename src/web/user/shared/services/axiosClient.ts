...
function shouldRetryTransient(error: {
  ...
  if (error.code === 'ECONNRESET' || error.code === 'ECONNABORTED') return true;
...
apiClient.interceptors.response.use(
  ...
  async (error) => {
    ...
    // Retry safe GETs while Django restarts (Vite proxy returns 503 for a few seconds).
    if (original && shouldRetryTransient(error)) {
