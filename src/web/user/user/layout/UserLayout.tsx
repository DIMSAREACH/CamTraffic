...
export function UserLayout() {
  ...
  useEffect(() => {
    if (!isLoading && user && user.role !== 'admin') {
      // Wrong domain → send to role home (officer vs citizen)
      if (user.role === 'police' && location.pathname.startsWith('/citizen')) {
        navigate(homePathForRole('police'), { replace: true });
        return;
      }
      if (user.role === 'driver' && location.pathname.startsWith('/officer')) {
        navigate(homePathForRole('driver'), { replace: true });
        return;
      }
