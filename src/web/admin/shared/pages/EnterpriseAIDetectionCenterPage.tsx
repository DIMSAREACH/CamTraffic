...
/** Admin portal AI routes (this page is only mounted under /admin). */
const ADMIN_AI_ROUTES = {
  aiDetection: '/admin/ai-detection',
...
export function EnterpriseAIDetectionCenterPage() {
  ...
  const selectInputMode = (mode: EnterpriseInputMode) => {
    ...
    setResult(null);
    setPreviewSrc((prev) => {
      // Delay revoke so unmounting <img>/<video> are gone before the blob is invalidated.
  ...
  const handleResult = (res: CenterDetectionResult, preview: string) => {
    setResult(res);
    // Prefer durable server/media URLs — never keep a child-owned blob that may be revoked.
    const next =
      res.uploaded_image ||
  ...
  return (
    <div className="enforcement-page enforcement-page--ai-center dashboard-home dashboard-page--ai-center enterprise-ai-page">
        ...
        <div className="enforcement-page__hero-inner enterprise-ai-page__hero-inner">
          ...
          <div className="enterprise-ai-page__hero-actions">
            <button
              ...
              onClick={() => navigate(ADMIN_AI_ROUTES.aiDetection)}
            >
              <ArrowLeft size={15} />
            ...
            <button
              type="button"
              className="enterprise-ai-page__action-btn"
              onClick={() => navigate(ADMIN_AI_ROUTES.aiLogs)}
            >
              <BarChart3 size={15} />
      ...
      <RecentDetectionsTable
        ...
        loading={loadingLogs}
        onViewAll={() => navigate(ADMIN_AI_ROUTES.aiLogs)}
        onViewLog={() => navigate(ADMIN_AI_ROUTES.aiLogs)}
