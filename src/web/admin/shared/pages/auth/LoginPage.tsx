    ...
    if (!isLoading && user) {
      if (IS_ADMIN_SURFACE) {
        if (user.role === 'admin') {
          ...
          const home = user.role === 'driver' ? '/citizen' : '/officer';
          window.location.assign(getUserDevUrl(home));
        }
        return;
      }
      if (user.role === 'police' || user.role === 'driver') {
        navigate(user.role === 'driver' ? '/citizen' : '/officer', { replace: true });
      }
    }
    ...
    try {
      ...
      const go = () => {
        if (loggedIn.role === 'admin') {
          ...
          const home = loggedIn.role === 'driver' ? '/citizen' : '/officer';
          if (IS_ADMIN_SURFACE) window.location.assign(getUserDevUrl(home));
