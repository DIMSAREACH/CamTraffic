...
import {
  ...
  MapPin, FileText, User, Hash, Car, BadgeCheck, CreditCard, Pencil, Trash2, ImageIcon,
} from 'lucide-react';
...
const STATUS_TABS = ['all', 'pending', 'awaiting_verification', 'paid', 'overdue', 'dismissed'] as const;
...
export function FineManagement() {
  ...
  const handleVerifyPayment = async (id: string, approve = true) => {
    try {
      const updated = await finesAPI.verifyPayment(id, approve);
      setFines((prev) => prev.map((f) => (f.id === id ? updated : f)));
      if (selected?.id === id) setSelected(updated);
      toast.success(
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      setSelected(updated);
      toast.success(
        updated.status === 'awaiting_verification'
          ? t('fines.toastAwaitingVerification')
          : t('fines.toastPaid'),
      );
  ...
  return (
    <div className="enforcement-page enforcement-page--fines dashboard-page--fines">
                    ...
                    <TableCell className="fines-table__td fines-table__td--actions">
                      <div className="fines-table__actions" role="group" aria-label={t('fines.colActions')}>
                        <button
                          ...
                          onClick={() => {
                            if (isDriver) {
                              navigate(`${USER_PORTAL_ROUTES.fines}/${row.id}`);
                              return;
                            }
                            setSelected(row);
                          }}
                        ...
                        {canManage && row.status === 'awaiting_verification' ? (
                          <>
                            <button
                              type="button"
                              className="fines-table__icon-btn fines-table__icon-btn--paid"
                              onClick={() => void handleVerifyPayment(row.id, true)}
                              title={t('fines.verifyPayment')}
                              aria-label={t('fines.verifyPayment')}
                            >
                            ...
                            <button
                              type="button"
                              className="fines-table__icon-btn fines-table__icon-btn--delete"
                              onClick={() => void handleVerifyPayment(row.id, false)}
                              title={t('fines.rejectPayment')}
                              aria-label={t('fines.rejectPayment')}
                            >
                              <XCircle size={16} strokeWidth={2.35} aria-hidden />
                            </button>
                          </>
                        ) : null}
      ...
      <Dialog open={paymentOpen} onOpenChange={setPaymentOpen}>
        <DialogContent accent="success" className="max-w-sm fines-pay-dialog">
          <DialogHeader>
          ...
          {selected && (
            <div className="fines-pay-dialog__body">
              <div className="fines-pay-dialog__amount">
                ...
                {khqrSession?.amount_usd ? (
                  <p className="fines-pay-dialog__amount-usd">
                    {t('fines.khqrAmountLabel')}: <strong>{khqrSession.amount_usd} USD</strong>
                  </p>
                ) : null}
              </div>
              ...
              {paymentModes.includes('khqr') ? (
                <div className="fines-pay-dialog__khqr">
                  ...
                  {khqrSession?.merchant_name ? (
                    <p className="fines-pay-dialog__merchant">
                      {t('fines.khqrTitle')} — {khqrSession.merchant_name}
                    </p>
                  ) : null}
                </div>
              ) : (
                <p className="fines-pay-dialog__empty text-sm text-[color:var(--dialog-subtitle-fg)]">
                  {formatFineReason(selected.reason)}
                </p>
              )}
            </div>
