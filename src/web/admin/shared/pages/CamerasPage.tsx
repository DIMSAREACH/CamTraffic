...
const TYPE_STYLE: Record<CameraType, { bg: string; color: string; border: string }> = {
  ...
  mobile: { bg: 'rgba(14, 165, 233, 0.12)', color: '#0284C7', border: 'rgba(14, 165, 233, 0.32)' },
...
function typeStyle(cameraType: string | undefined) {
  return TYPE_STYLE[(cameraType as CameraType) || 'fixed'] ?? TYPE_STYLE.fixed;
}
                ...
                <div className="ct-dialog-field">
                  ...
                  >
                    <SelectTrigger
                      ...
                      style={{
                        backgroundColor: typeStyle(form.camera_type).bg,
                        color: typeStyle(form.camera_type).color,
                        borderColor: typeStyle(form.camera_type).border,
                      }}
                    >
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div className="ct-dialog-field">
                  ...
                  >
                    <SelectTrigger
                      className="cameras-form-dialog__select cameras-form-dialog__select--tone"
                      style={{
                        backgroundColor: statusMeta(form.status).bg,
                        color: statusMeta(form.status).color,
                        borderColor: statusMeta(form.status).border,
                      }}
                    >
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent className="cameras-form-dialog__select-menu">
                      {(['active', 'inactive', 'offline', 'maintenance'] as const).map((status) => (
...
function CameraFeedPreview({
  ...
  const handleError = () => {
    ...
    // Demo frame fallback only when explicitly allowed (never pretend live CCTV).
  ...
  const isLive = isStreaming && autoRefresh && !isDemo;
    ...
    <div className={cn('cameras-preview-body cameras-preview-body--fill', isLive && 'cameras-preview-body--live')}>
      <div className="cameras-preview-toolbar cameras-preview-toolbar--compact">
        <div className="cameras-preview-toolbar__status">
          {isStreaming && isDemo && (
            <span className="cameras-live-badge cameras-live-badge--demo">
              {t('pages.cameras.demoFrameBadge')}
            </span>
          )}
          {isLive && (
      ...
      <div className={cn(
        ...
        <div className="cameras-feed-chrome" aria-hidden={feedState === 'loading'}>
          <div className="cameras-feed-chrome__top">
            {isStreaming && isDemo ? (
              <span className="cameras-feed-chip cameras-feed-chip--demo">
                {t('pages.cameras.demoFrameBadge')}
              </span>
            ) : isLive ? (
...
export function CamerasPage() {
  ...
  const handleDeleteCamera = async (camera: Camera) => {
    ...
    try {
      ...
      const remaining = cameras.filter((c) => c.id !== camera.id);
