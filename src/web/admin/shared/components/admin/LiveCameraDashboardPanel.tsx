  ...
  if (cameras.length === 0) {
    return (
      <div className={cn('cameras-live-overview', compact && 'cameras-live-overview--compact')}>
        ...
        <div className="cameras-live-overview__empty" role="status">
          <div className="cameras-live-overview__empty-icon" aria-hidden>
            <Cctv size={22} strokeWidth={1.75} />
          </div>
          <p className="cameras-live-overview__empty-title">{t('liveCameraDashboard.empty')}</p>
          <p className="cameras-live-overview__empty-hint">{t('pages.cameras.selectCameraHint')}</p>
        </div>
      </div>
    );
  }
