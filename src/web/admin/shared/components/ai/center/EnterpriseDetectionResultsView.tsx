...
export function EnterpriseDetectionResultsView({
  ...
  // Prefer raw image + CSS overlay. Never stack baked OpenCV boxes with overlay (double annotation).
  const overlayReady = useMemo(
    () => buildDetectionOverlay(result as OverlayDetectionInput, speechLocale).length > 0,
    [result, speechLocale],
  );
  const displaySrc = overlayReady
    ? (result.uploaded_image || previewSrc || result.processed_image || result.annotated_processed_image || '')
    : (result.annotated_processed_image || result.processed_image || result.uploaded_image || previewSrc || '');
          ...
          <div className="enterprise-ai-results__image-body">
            {displaySrc ? (
              <AnnotatedDetectionImage
                ...
                result={overlayReady ? (result as OverlayDetectionInput) : null}
                showOverlay={overlayReady}
                hero
              />
            ) : (
