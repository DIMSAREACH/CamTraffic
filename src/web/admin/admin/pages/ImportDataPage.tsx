...
import {
  Camera, Car, CheckCircle2, Database, Download, Eye, FileSpreadsheet, FileUp, History, Loader2,
...
import type {
  ImportDataType, ImportJobDetail, ImportJobSummary, ImportRowReport, ImportValidateCounts, ImportValidateResult,
} from '@shared/types';

const ACCEPT_EXT = /\.(csv|xlsx|xlsm)$/i;
...
function isImportSpreadsheet(file: File): boolean {
...
export function ImportDataPage() {
  ...
  const assignFile = useCallback((next: File | null) => {
    ...
    if (!isImportSpreadsheet(next)) {
      toast.error(t('importData.invalidFileType'));
      return;
    }
  ...
  const openHistoryDetail = async (jobId: string) => {
    setDetailLoading(true);
    try {
      setDetailJob(await importsAPI.historyDetail(jobId));
    } catch (err: unknown) {
      toast.error(err instanceof Error ? err.message : t('importData.historyFailed'));
    } finally {
  ...
  return (
    <div className="enforcement-page enforcement-page--import">
      ...
      <section className="enforcement-page__panel import-page__panel mb-6">
        ...
        <div className="import-page__upload-row">
          <div
            ...
            onDrop={onDrop}
            aria-label={t('importData.dropZoneAria')}
          >
            <span className="import-page__file-drop-icon" aria-hidden>
              {file ? <FileSpreadsheet size={28} strokeWidth={1.75} /> : <FileUp size={28} strokeWidth={1.75} />}
            </span>
            <span className="import-page__file-drop-copy">
              <strong>
                {dragging
                  ? t('importData.dropNow')
                  : file
                    ? file.name
                    : t('importData.dropTitle')}
              </strong>
              <span>
                {file
                  ? `${(file.size / 1024).toFixed(1)} KB · ${t('importData.dropReplace')}`
                  : t('importData.fileHint')}
              </span>
              {!file && !dragging ? (
                <span className="import-page__file-drop-browse">{t('importData.browseFiles')}</span>
              ) : null}
            </span>
            <input
              ref={fileInputRef}
            ...
            <Table className="enforcement-page__table">
              <TableHeader>
                <TableRow className="enforcement-page__table-head">
                  ...
                  <TableHead className="enforcement-page__th text-right">{t('common.actions')}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {history.map((job) => {
                  ...
                  return (
                    <TableRow key={job.id} className="enforcement-page__table-row">
                      ...
                      <TableCell className="text-right">
                        <button
                          type="button"
                          className="enforcement-page__icon-btn"
                          onClick={() => void openHistoryDetail(job.id)}
      ...
      <Dialog open={!!detailJob} onOpenChange={(open) => !open && setDetailJob(null)}>
        <DialogContent accent="violet" className="max-w-3xl sm:max-w-3xl">
          <DialogHeader>
            <DialogTitle>{t('importData.historyDetailTitle')}</DialogTitle>
          </DialogHeader>
          {detailJob ? (
            <div className="space-y-4">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
                <div>
                  <p className="text-muted-foreground">{t('importData.colType')}</p>
                  <p className="font-medium">{detailJob.import_type}</p>
                </div>
                <div>
                  <p className="text-muted-foreground">{t('importData.colStatus')}</p>
                  <p className="font-medium">{statusLabel(detailJob.status)}</p>
                </div>
                <div>
                  <p className="text-muted-foreground">{t('importData.colFile')}</p>
                  <p className="font-medium truncate" title={detailJob.file_name}>{detailJob.file_name}</p>
                </div>
                <div>
                  <p className="text-muted-foreground">{t('importData.colWhen')}</p>
                  <p className="font-medium">{formatWhen(detailJob.created_at)}</p>
                </div>
              </div>
              <div className="import-page__table-wrap max-h-80 overflow-auto">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead>#</TableHead>
                      <TableHead>{t('importData.colStatus')}</TableHead>
                      <TableHead>{t('importData.colSummary')}</TableHead>
                      <TableHead>{t('importData.colErrors')}</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {(detailJob.rows_report || []).slice(0, 200).map((row) => {
                      const meta = statusMeta(row.status);
                      return (
                        <TableRow key={`${detailJob.id}-${row.row}`}>
                          ...
                          <TableCell className="text-sm">
                            {formatRowSummary(detailJob.import_type, row.data || {})}
                          </TableCell>
