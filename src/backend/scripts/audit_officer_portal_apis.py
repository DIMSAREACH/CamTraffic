...
def main() -> int:
    ...
    with override_settings(ALLOWED_HOSTS=['*']):
        ...
        # Live detection against a camera with a resolvable non-demo frame
        live_cam = None
        for cam in Camera.objects.filter(status='active').order_by('code'):
            url = (cam.effective_frame_url() if hasattr(cam, 'effective_frame_url') else cam.frame_source_url) or ''
            if not url or 'demo-cameras' in url:
                continue
            if url.startswith('/') and not resolve_local_frame_path(url):
                continue
            live_cam = cam
            break
