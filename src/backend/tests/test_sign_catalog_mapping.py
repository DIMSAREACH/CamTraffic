...
from tests.catalog_helpers import assert_sign_code, use_full_sign_catalog, use_production_catalog
...
class SignCatalogMappingTest(SimpleTestCase):
    ...
    def test_result_from_class_key_uses_real_catalog_name(self):
        with use_full_sign_catalog():
            ...
            self.assertEqual(result['sign_code'], 'I-036')
            self.assertEqual(result['class_key'], 'i_no_trucks')
    ...
    def test_result_from_class_key_stop_uses_catalog(self):
        with use_full_sign_catalog():
            ...
            self.assertEqual(result['class_key'], 'i_stop')
            self.assertTrue(result.get('sign_name_en') or result.get('sign_name'))
    ...
    def test_resolve_official_sign_labels_fixes_placeholder(self):
        with use_production_catalog():
            ...
            assert_sign_code(self, fixed, 'PW03-R1-01', 'R1-01', 'PROH-002')
