...
def channel_status() -> dict:
    ...
    push_ok = fcm_enabled() or web_push_enabled()
    sms_ok = sms_enabled()
    return {
        'system': True,
        'email': email_ok,
        ...
        'sms': sms_ok,
        'configured': {
            'resend': bool(getattr(settings, 'RESEND_API_KEY', '').strip()),
            'smtp': bool(getattr(settings, 'EMAIL_HOST_USER', '').strip()),
            'fcm': bool(getattr(settings, 'FCM_SERVER_KEY', '').strip()),
            'web_push': bool(getattr(settings, 'VAPID_PUBLIC_KEY', '').strip()),
            'twilio': bool(
        ...
        'production_ready': True,  # in-app always works; outbound channels optional
        'outbound_ready': email_ok or push_ok or sms_ok,
    }
