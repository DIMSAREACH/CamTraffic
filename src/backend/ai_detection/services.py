...
def _dataset_class_from_filename(name: str) -> str | None:
    """
    Resolve YOLO/catalog class from dataset-style names:
    I_AIRPORTS_Airports_01.jpg → i_airports
    I_PARKING_LOT_Parking lot_00.jpg → i_parking_lot
    """
    stem = Path(name).stem
    ...
    for part in parts:
        if not part:
            continue
        if re.fullmatch(r'[IWP]', part) or re.fullmatch(r'PW\d+', part, flags=re.I):
...
def _explicit_filename_class(hint_source: str) -> str | None:
    ...
    if stem_class:
        return stem_class
    dataset_class = _dataset_class_from_filename(basename)
    if dataset_class:
        row = _catalog_row_for_token(dataset_class)
        if row and row.get('class_key'):
            return _canonical_class_key(row['class_key'])
...
def _is_live_capture_filename(name: str) -> bool:
    stem = Path(name).stem.lower()
    return (
        stem.startswith('webcam-')
        or stem.startswith('camera-frame')
        or stem.startswith('camera_')
        or stem == 'frame'
    )
...
def _yolo_raw_detect(image_path, hint_source: str | None = None, *, live_fast: bool = False, unified_prep: bool = False):
    ...
    # Filename decides live vs upload. unified_prep only means the image was already
    # preprocessed — it must not force live-strict 0.50 infer (that zeroes real uploads).
    ...
    def _usable_raw(raw: dict | None) -> dict | None:
        ...
        if live_capture:
            ...
            return raw
        if unified_prep:
            # Shared preprocess can drop box scores; keep upload floor (not live trust).
            if conf < _upload_yolo_floor():
                return None
            return raw
        return raw

    live_strict = live_capture
