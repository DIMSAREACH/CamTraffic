...
def _apply_ocr_character_fixes(cleaned: str) -> str:
    ...
    for ch in cleaned:
        ...
        if not seen_letter and not in_serial:
            ...
            out.append(ch)
            continue
        # Serial number (after dash, or digits after letter block without dash yet)
        serial_zone = in_serial or (seen_letter and ch.isdigit())
        if in_serial or (seen_letter and len(out) >= 2 and out[-1].isdigit() and '-' in out):
            serial_zone = True
        if in_serial:
            if ch in ('O', 'Q', 'D'):
                out.append('0')
                ...
                out.append(ch)
            continue
        # Letter block: 0 misread as letter O already handled above; keep alphas
        if ch == '0' and out and out[-1].isalpha():
            out.append('O')
        else:
            out.append(ch)
    return ''.join(out)
...
def normalize_plate_text(text: str) -> str | None:
    ...
    # Second pass after dash insertion so serial OCR confusions (B→8, O→0) apply
...
def _read_text_from_image(image_bgr: np.ndarray, region: str) -> list[dict]:
    ...
    for variant in _enhance_for_ocr(image_bgr):
        ...
        if best and best['confidence'] >= early_exit:
            return reads
    return reads
