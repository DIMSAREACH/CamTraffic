...
def sign_class_key_candidates(class_key: str) -> list[str]:
    ...
    if stripped and stripped not in out:
        out.append(stripped)
    # no_u_turn already primary; also try with common prefixes removed twice
...
def evaluate_violation(
    *,
    ...
    for candidate in sign_class_key_candidates(class_key):
...
def get_violation_stats(queryset=None):
    qs = queryset if queryset is not None else TrafficViolation.objects.all()
    return {
