"""Tests for the production Cambodian traffic sign catalog (50 classes)."""
...
class TrafficSignCatalog10Tests(SimpleTestCase):
    def test_catalog_file_exists_with_ten_signs(self):
        self.assertTrue(CATALOG_PATH.is_file(), 'Missing ai/traffic_sign_catalog_10.json')
        payload = json.loads(CATALOG_PATH.read_text(encoding='utf-8'))
        signs = payload['signs']
        self.assertEqual(len(signs), 50)
        self.assertEqual(payload['total_classes'], 50)
        self.assertEqual(payload['model'], 'YOLOv8')
        self.assertIn('50', str(payload.get('dataset', '')))
    ...
    def test_no_entry_metadata(self):
        ...
        self.assertEqual(no_entry['sign_code'], 'PROH-001')
    ...
    def test_loader_normalizes_thesis_fields(self):
        ...
        if resolve_catalog_path().name != 'traffic_sign_catalog_10.json':
            self.skipTest('Production catalog not active')
    ...
    def test_loader_returns_rows_for_all_class_keys(self):
        ...
        self.assertTrue(CORE_CLASS_KEYS.issubset(keys))
