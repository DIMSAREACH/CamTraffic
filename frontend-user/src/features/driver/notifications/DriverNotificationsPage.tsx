...
import type { DriverNotificationRecord, DriverNotificationSummary } from '@camtraffic/types';

interface DriverNotificationsPageProps {
  onLoadSummary: () => Promise<DriverNotificationSummary>;
  onLoadList: (params?: { search?: string; is_read?: string; limit?: number }) => Promise<DriverNotificationRecord[]>;
  onLoadDetail: (notificationId: number) => Promise<DriverNotificationRecord>;
  onUpdate: (notificationId: number, isRead: boolean) => Promise<{ notification: DriverNotificationRecord; message: string }>;
...
export function DriverNotificationsPage({
  ...
  const [summary, setSummary] = useState<DriverNotificationSummary | null>(null);
  const [records, setRecords] = useState<DriverNotificationRecord[]>([]);
  const [selectedId, setSelectedId] = useState<number | null>(null);
  const [detail, setDetail] = useState<DriverNotificationRecord | null>(null);
  ...
  async function handleToggleRead(notification: DriverNotificationRecord) {
    setSaving(true);
    setErrorMessage(null);
  ...
  return (
    <Card title={t.nav.notifications} subtitle="Task 081 — Driver notification center">
      {summary ? (
