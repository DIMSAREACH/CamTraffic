"""Expand Tuk Tuk box to full vehicle (include plate 1HU-3055)."""
...
# Full white/black tuk-tuk including canopy, body, and rear plate
TUK_XYXY = [2840, 930, 3460, 1340]
MOTO_XYXY = [3485, 1005, 3655, 1245]
FAR_TUK_XYXY = [3660, 1020, 3839, 1220]


def to_fields(xyxy: list[int], w: int, h: int) -> tuple[dict, dict]:
    x1, y1, x2, y2 = xyxy
    bbox_norm = {
        'x1': round(x1 / w, 6),
        'y1': round(y1 / h, 6),
...
def main() -> None:
    ...
    for a in meta['annotations']:
        ...
        is_tuk_or_moto = 'tuk' in label or 'tuk' in key or 'moto' in label or 'moto' in key
        if is_tuk_or_moto and x1 >= 2750:
            print('Removed', a.get('label_en'), xy)
            continue
        keep.append(a)

    for label, key, cid, conf, xyxy in (
        ('Tuk Tuk', 'TUK_TUK', 104, 92.0, TUK_XYXY),
        ('Motorcycle', 'MOTORCYCLE', 101, 88.0, MOTO_XYXY),
        ('Tuk Tuk', 'TUK_TUK', 104, 75.0, FAR_TUK_XYXY),
    ):
        bbox_norm, bbox_yolo = to_fields(xyxy, w, h)
        keep.append({
            'kind': 'vehicle',
            'class_id': cid,
            'class_key': key,
    ...
    meta['notes'] = (
        'Height Limit 5.5m on overpass. Full Tuk Tuk box (incl. plate 1HU-3055) '
        '+ Motorcycle + far-right Tuk Tuk.'
    )
    ...
    # QA crop of expanded tuk
    x1, y1, x2, y2 = TUK_XYXY
    cv2.imwrite(str(OUT / '_qa_tuk_full.jpg'), canvas[y1:y2, x1:x2])
